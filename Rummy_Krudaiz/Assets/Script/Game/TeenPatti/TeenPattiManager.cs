...
public class TeenPattiManager : MonoBehaviour
{
    ...
    [Header("---Game Play---")] public int gameDealerNo;
    ...
    [Header("--- Prefab ---")] public GameObject targetBetObj;
    ...
    [Header("--- Chat Panel ---")] public GameObject chatPanelParent;
    public GameObject chatMePrefab;
    public GameObject chatOtherPrefab;

    [Header("--- Game UI ---")] public GameObject errorScreenObj;
    public GameObject slideShowPanel;
    public Text totalPriceTxt;
    public GameObject bottomBox;
    public Button peckButton;
    public Button showButton;
    public Button blindButton;
    public Button blindInButton;
    public Button blindOutButton;
    ...
    [Header("--- Gift Maintain ---")] public GameObject giftParentObj;
    public GameObject giftPrefab;
    public List<GiftBox> giftBoxes = new List<GiftBox>();

    [Header("--- Game Data Maintain ---")] public float totalBetAmount;
    ...
    public void CheckNewPlayers()
    {
        ...
        DataManager.Instance.joinPlayerDatas =
            DataManager.Instance.joinPlayerDatas.Where(player => !player.avtar.StartsWith("http://167.71.234.194/assets/img/profile-picture/")).ToList();
    ...
    public void SetFoolterButton(bool isActive)
    {
        peckButton.interactable = isActive;
        showButton.interactable = isActive;
        blindButton.interactable = isActive;
        blindInButton.interactable = isActive;
        blindOutButton.interactable = isActive;
    }

    IEnumerator DataMaintain()
    {
        Debug.Log(" DataMainCall");
        playerSquList.Clear();
    ...
    public void ClosePopup()
    {
        if (!isPopupOpen) return;
        isPopupOpen = false;
        sideShowPopupImage.transform.DOScale(0f, 0.3f).SetEase(Ease.InBack).OnComplete(() => { sideShowPopupImage.gameObject.SetActive(false); });
    }
    ...
    public void ChipGenerate(GameObject chip, Vector3 endPos)
    {
        ...
        {
            chip.transform.DOScale(new Vector3(0.8f, 0.8f, 0.8f), 0.1f).OnComplete(() => { chip.transform.DOScale(Vector3.one, 0.07f); });
        });
    }
    ...
    public void GetGift(string sendPlayerID, string receivePlayerId, int giftNo)
    {
        ...
        giftGen.transform.DOMove(receivePlayerObj.transform.position, 0.4f).OnComplete(() =>
        {
            ...
            {
                giftGen.transform.DOScale(Vector3.zero, 0.5f).OnComplete(() => { Destroy(giftGen); });
            });
        });
            ...
            if (teenPattiPlayers[j].playerId == playerID && teenPattiPlayers[j].gameObject.activeSelf == true)
            {
                ...
                if (teenPattiPlayers[j].playerNo == player1.playerNo)
                {
                    ...
                    if (playerWinAmount != 0)
                    {
                        ...
                        DataManager.Instance.AddAmount((float) (playerWinAmount), DataManager.Instance.gameId, "TeenPatti-Win-" + DataManager.Instance.gameId, "won",
                            (float) (adminCommssion), player1.playerNo);
                    }
                }
    ...
    public IEnumerator CheckSlideShowWinner(string playerId1, string playerId2, bool isSocket)
    {
        ...
        for (int i = 0; i < teenPattiPlayers.Count; i++)
        {
            if (isSocket && teenPattiPlayers[i].gameObject.activeSelf == true && teenPattiPlayers[i].isPack == false &&
                (teenPattiPlayers[i].playerId.Equals(playerId1) || teenPattiPlayers[i].playerId.Equals(playerId2)))
            {
                cnt++;
        ...
        print("______________________________This is called From outside the loop_______________________________________________");
        if (CheckMoney(currentPriceValue) == false)
        {
            SoundManager.Instance.ButtonClick();
            OpenErrorScreen();
    ...
    public string CheckFinalWinner(string type)
    {
        ...
        for (int i = 0; i < sortedNumbersDescending.Count; i++)
        {
            ...
            if (sortedNumbersDescending[i].gameObject.activeSelf == true && (sortedNumbersDescending[i].isSeen || sortedNumbersDescending[i].isBlind) &&
                sortedNumbersDescending[i].isPack == false)
            {
                bool isEnter = false;

...
public class CashFreeManage : MonoBehaviour
{
    ...
    public IEnumerator getToken(int amount, string couponId11)
    {
        ...
        UnityWebRequest unityWeb = UnityWebRequest.Post(DataManager.Instance.url + "/api/v1/payments/phonepaypg/token", tokeform);
    ...
    private void ProcessTokenData(string tokenJson)
    {
        JSONNode token = JSON.Parse(tokenJson);

        if (token["success"].AsBool)
        {
    ...
    private void OnApplicationFocus(bool focus)
    {
        if (focus)
        {
            if (isRun)
            {
                ...
                //StartCoroutine(OnClearCacheClicked());
                print("isRun");
            }
            else if (isRun1)    
            {
    ...
    {
        WWWForm form = new WWWForm();
        form.AddField("orderId", transactionId);
        ...
        if (node["success"] == true)
        {
            ...
            print("Cashfree Data Balance 1: " + (float.Parse(data["balance"].ToString()) * 1));

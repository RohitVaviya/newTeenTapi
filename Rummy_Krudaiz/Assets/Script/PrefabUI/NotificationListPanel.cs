...
public class NotificationListPanel : MonoBehaviour
{
    ...
    {
        var request = UnityWebRequest.Get(DataManager.Instance.url + "/api/v1/notifications/player");
        ...
        if (request.error == null && !request.isNetworkError)
        {
            ...
            {
                noNotificationText.text = "No Notifications available";
                MainMenuManager.Instance.notificationRedDot.SetActive(false);
                scrollObj.SetActive(false);
            }
            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                var sendTo = item["sendTo"].Value;
                var isForPlayer = sendTo == "player";

                if (isForPlayer)
                {
                    var playerNotifications = item["player_not"];
                    var playerMatchFound = false;

                    for (var j = 0; j < playerNotifications.Count; j++)
                    {
                        var playerNotification = playerNotifications[j];
                        var playerId = playerNotification["playerId"].Value;
                    ...
                    if (!playerMatchFound)
                        continue; // Skip this notification if not intended for the player
                }

                var temp = Instantiate(notificationbar, scrollParent.transform);
                MainMenuManager.Instance.notiBarManages.Add(temp.GetComponent<NotiBarManage>());

                var title = item["title"].Value;
                var message = item["message"].Value;
                var url = item["url"].Value;
                var imageUrl = item["imageUrl"].Value;
                var createdAt = item["createdAt"].Value;

                var notiBarManage = temp.GetComponent<NotiBarManage>();

                temp.GetComponent<Button>().onClick.AddListener(() => NotificationsButtonClick(url));

                notiBarManage.titleTxt.text = title;
                notiBarManage.middleTitleTxt.text = message;

                // Add your date formatting logic here
                var createdAtDateTime = DateTime.Parse(createdAt);
                var formattedDate = createdAtDateTime.ToString("yyyy-MM-dd HH:mm:ss");
                notiBarManage.dateTxt.text = formattedDate;

                StartCoroutine(GetImages(DataManager.Instance.url + "/api/v1/files/" + imageUrl,

...
public class TestSocketIO : MonoBehaviour
{
    ...
    public void HandelEvents(SocketIOEvent data)
    {
        ...
        switch (ev)
        {
            ...
            case "notification_player":
                Noti_player(data.data.ToString());
                break;
    ...
    public void Noti_player(string json)
    {
        JSONNode node = JSON.Parse(json);
        JSONNode data = JSON.Parse(node["data"].ToString());
        print("Single notificatio is called.");

        if (data["PlayerID"] == DataManager.Instance.playerData._id)
        {
